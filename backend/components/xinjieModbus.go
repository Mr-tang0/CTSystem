...
// 读取32位HD整数
func (x *XinjieClient) ReadInt32HD(address uint16, count uint16) []int32 {
	registers, err := x.ReadRegisters(address+0xA080, count*2)
	if err != nil {
		return nil
	}
	...
	for i := 0; i < int(count); i++ {
		startIndex := i * 4
		w0 := binary.BigEndian.Uint16(registers[startIndex : startIndex+2])
		w1 := binary.BigEndian.Uint16(registers[startIndex+2 : startIndex+4])

		// 信捷PLC HD寄存器存储顺序: 低字在前，高字在后
		data := uint32(w1)<<16 | uint32(w0)

		results[i] = int32(data)
	}
	return results
}

// 读取64位HD整数
func (x *XinjieClient) ReadInt64HD(address uint16, count uint16) []int64 {
	registers, err := x.ReadRegisters(address+0xA080, count*4)
	if err != nil {
		return nil
	}
	...
	for i := 0; i < int(count); i++ {
		...
		w0 := binary.BigEndian.Uint16(registers[startIndex : startIndex+2])
		w1 := binary.BigEndian.Uint16(registers[startIndex+2 : startIndex+4])
		w2 := binary.BigEndian.Uint16(registers[startIndex+4 : startIndex+6])
		w3 := binary.BigEndian.Uint16(registers[startIndex+6 : startIndex+8])

		// 信捷PLC HD寄存器存储顺序: 低字在前，高字在后
		data := uint64(w3)<<48 | uint64(w2)<<32 | uint64(w1)<<16 | uint64(w0)

		results[i] = int64(data)
	}
...
// PackAndWriteCoils：将 bool 数组打包成字节并写入
